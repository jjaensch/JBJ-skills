...
2. List files in the relevant reference subdirectory — Advanced-guide, User-guide, API-references — then read the most applicable file(s).
3. Construct the answer using version 1.1.0 APIs.
4. If the answer involves packages beyond GeoPandas, name them explicitly.
5. **Cite sources (MANDATORY):** At the end of every response, add a `### References` section listing each reference file that was consulted. Use a markdown link to the file path relative to the skill root, e.g.:
   ```
   ### References
   - [Making-maps-and-plots.md](references/User-guide/Making-maps-and-plots.md)
   - [GeoDataFrame.md](references/API-references/GeoDataFrame.md)
   ```
   Do NOT skip this step — even for short or conversational answers.


### Notes / Constraints 
- **Every response MUST end with a `### References` section** listing the consulted reference files. No exceptions.
